...
def parser_node(state: AgentState) -> AgentState:
    """
    Parser agent node that runs log searches / GC analysis and appends evidence.
    It expects `last_generated_keywords` to be present (set by analyzer) or will fall back
    to `keywords` in state.
    Returns partial state updates: evidence (appended), last_logs_chunk, and clears next_action.
    The graph will automatically route back to reasoning via edge.
    """
    ...
    try:
        logs_chunk = parser_fn(logs_path, kw, hint="agent-parser")
    except Exception as e:
        logs_chunk = (
            f"[ERROR] path={logs_path}\nkeywords={kw}\nError in parser agent: {e}"
        )

    # Append evidence while preserving order and deduping
    evidence = list(state.get("evidence") or [])
    evidence.append(logs_chunk)

    # Clear last_generated_keywords to avoid accidental reuse
    return {
        "evidence": evidence,
        "last_logs_chunk": logs_chunk,
        "last_generated_keywords": [],
        # Parser returns control to reasoning automatically via graph edge
    }
